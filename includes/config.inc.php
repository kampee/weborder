...
$wb_remote_client = GETENV("REMOTE_HOST");
...
$wb_tbl_ulog="wb_ulog"; // Login Session
$wb_tbl_brand="wb_brand"; // Brand
$wb_tbl_wwkmadein="wb_wwkmadein"; // WWK Made In
$wb_tbl_wwkcode="wb_wwkcode"; // WWK Code
$wb_tbl_wwksize="wb_wwksize"; // WWK Size
$wb_tbl_incoterms="wb_incoterms"; // Incoterms
$wb_tbl_ictprice="wb_ictprice"; // Incoterms Price


// Forgot password
$forgot_password_subject = "Your email & password";
$wb_contact_email = "kampee@clwglobal.com";
...
$std_font_mains="arial, ms sans serif, Tahoma, sans-serif";
...
function errmessnrt($messages){
	echo "<div align=left>";
	echo "<font face=\"Tahoma,ms sans serif,verdana,arial\" size=\"2\" color=\"red\">";
	echo "<b>$mess</b>"."<br>";
	echo "</font>";
	echo "</div>";
	echo "<br>";
...
function errmess($mess){
	...
	echo "<font face=\"ARIAL,Tahoma,ms sans serif,verdana\" size=\"2\" color=\"red\">";
	echo "<b>$mess</b>"."<br>";
...
function LoginAuth()
{
	...
	session_register("session_stk");
	session_register("session_ulog");
	session_register("session_bid");
...
function PageList($datafound,$linkmaster)
{
	if($datafound>0)
	{
		...
		if ($web_found>$pages)
		{
			...
			echo "		<td>";
			echo "			<b><font size=\"2\" face=\"Tahoma, Microsoft Sans Serif\" color=\"RED\"><font color=black>Page : </font>";

			if ($startpage==$pages)
			{
				$pageprev=0;
				echo "<a href=\"".$linkmaster."&startpage=$pageprev&stoppage=$stoppage\"><font size=\"2\" face=\"Tahoma, Microsoft Sans Serif\" color=\"RED\">Previous</font></a> ";
			}
			...
			if ($startpage==0)
			{
				echo "<font size=\"2\" face=\"Tahoma, Microsoft Sans Serif\" color=\"RED\"><b>1</b></font>&nbsp;";
			}
			else
			{
			...
			for ($x>=$pages; $x<=$web_found; $x)
			{
				...
				if ($startpage==$x)
				{
					echo "<font size=\"2\" face=\"Tahoma, Microsoft Sans Serif\" color=\"RED\"><b>".(($x/$pages)+1)."</b></font>&nbsp;";
				}
				else
				{
					if ($pagereal<=$pageall)
			...
			if ($startpage<$x-$pages)
			{
				...
				echo "<a href=\"".$linkmaster."&startpage=$pagenext&stoppage=$stoppage\"><font size=\"2\" face=\"Tahoma, Microsoft Sans Serif\" color=\"RED\">Next</font></a>";
			}
...
function chkcursy($sy,$price,$dx)
{
	...
	switch($sy)
	{
		case "EUR":
			echo "<font color=\"red\">";
			echo "&euro; ".number_format($price,$dx);
			echo "</font>";
			break;
		case "USD":
			echo "<font color=\"red\">";
			echo "US $ ".number_format($price,$dx);
			echo "</font>";
			break;
		case "HKD":
			echo "<font color=\"red\">";
			echo "HK $ ".number_format($price,$dx);
			echo "</font>";
			break;
		case "POUND":
			echo "<font color=\"red\">";
			echo "&pound; ".number_format($price,$dx);
			echo "</font>";
			break;
	}
	//echo "</b>";
...
// check currency and prices 1-4
function addcnp($s_cur,$c_cur,$c_cur1,$c_cur2,$c_cur3,$c_price,$c_price1,$c_price2,$c_price3)
{
	...
	switch($s_cur)
	{
		case "POUND":
			$a_cur="POUND";
			if(($c_cur==$a_cur) and (!empty($c_price)))
			{
				$a_price=$c_price;
			}
			if(($c_cur1==$a_cur) and (!empty($c_price1)))
			{				
				$a_price=$c_price1;
			}				
			if(($c_cur2==$a_cur) and (!empty($c_price2)))
			{				
				$a_price=$c_price2;
			}				
			if(($c_cur3==$a_cur) and (!empty($c_price3)))
			{				
				$a_price=$c_price3;
			}				
			break;
...
function addcnp($s_cur,$c_cur,$c_cur1,$c_cur2,$c_cur3,$c_price,$c_price1,$c_price2,$c_price3)
{
	...
	for($i=0; $i<4; $i++)
	{
		...
		case $cur_list[$i]:
			...
			if($c_cur2==$cur_list[$i])
			{				
				$a_price=$c_price2;
			}				

			if($c_cur3==$cur_list[$i])
			{				
				$a_price=$c_price3;
			}				
